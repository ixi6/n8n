...
const COMMUNITY_NODE_ROOT = resolve(
	__dirname,
	'../../../../../community-nodes/n8n-nodes-jsonplaceholder',
);

const COMMUNITY_NODE_SOURCE = join(
	COMMUNITY_NODE_ROOT,
...
function createMockContext(params: Record<string, unknown> = {}): ExecuteContext {
	...
	return {
		...
		getNode: () => ({
			name: 'JSONPlaceholder',
			type: 'n8n-nodes-jsonplaceholder.jsonPlaceholder',
			parameters: params,
		}),
...
describe('executeSandboxed', () => {
	it('should execute a community node in sandbox and return data', async () => {
		const nodeType = createMockNodeType(COMMUNITY_NODE_SOURCE, 'JsonPlaceholder');
		const context = createMockContext({ resource: 'posts', operation: 'get', id: 1 });
