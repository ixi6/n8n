...
/**
 * Executes a community node inside a secure-exec V8 isolate.
 *
 * The node's compiled JS source is loaded into an in-memory filesystem,
 * its npm dependencies are projected via the module-access overlay, and
 * a minimal context shim bridges core IExecuteFunctions methods.
 *
 * Uses exec() mode so the async Promise from execute() becomes
 * __scriptResult__, which secure-exec awaits before releasing the context.
 * The serialized result is streamed back via stdout with a marker prefix.
 */
...
/**
 * Generates the JavaScript code that runs inside the V8 isolate.
 *
 * The last expression is the Promise from execute().then(), which tells
 * secure-exec's awaitScriptResult to wait for the async work to complete
